...
int main(int argc, char *argv[])
{
    ...
    ivi w2;
    w2.move(w.pos().rx() + w.width() + 75, 0);
    w2.setWindowFlags(Qt::FramelessWindowHint);
    w2.show();

    cont w3;
    w3.move(0, w.pos().ry() + w.height() + 10);
    ...
    QObject::connect(&w3, SIGNAL(startSystem()), &w3, SLOT(connectDevice()));
    QObject::connect(&w3, SIGNAL(endSystem()), &w3, SLOT(disconnectDevice()));
